...
The extension always saves to `./nanobanana-output/` (hardcoded). After every generation, move the output to the permanent location:

```bash
mv nanobanana-output/*.png Content/images/ 2>/dev/null; mv nanobanana-output/*.jpg Content/images/ 2>/dev/null
```

Final images live in `Content/images/`.

## Presenting Results

After generation completes:
1. Move files from `nanobanana-output/` to `Content/images/`
2. List `Content/images/` to confirm the file is there
3. Present the most recent image(s) to the user using the Read tool
